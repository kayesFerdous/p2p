...
pub async fn open_gossip_room(name: Option<String>) -> Result<()> {
    ...
    println!("Gossip room created successfully!");
    clipboard(ticket.to_string());
    println!("Ticket copied to clipboard.");
    println!("\nShare this ticket with others to join:");
    println!("\nThey can join by running:");
    println!("zerogate gossip -n [name] join -t [ticket]");
...
async fn subscribe_loop(
    ...
    while let Some(event) = receiver.try_next().await? {
        if let Event::Gossip(GossipEvent::NeighborUp(_node_id)) = &event {
        ...
        if let Event::Gossip(GossipEvent::Received(msg)) = event {
            match Message::from_bytes(&msg.content)? {
                Message::AboutMe { from, name } => {
                    ...
                    println!("> {} has joined tha gossip", name);
                }
                Message::Message {
                    from,
                    text,
